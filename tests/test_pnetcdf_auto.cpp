/**
 * Parallel NetCDF (PNetCDF) Distribution-Aware I/O Tests
 *
 * Tests the unified PNetCDF I/O API:
 * - write_pnetcdf_auto()
 * - read_pnetcdf_auto()
 * - Distributed and replicated modes
 *
 * Run with: mpirun -np 4 ./test_pnetcdf_auto
 */
...
int main(int argc, char** argv) {
  ...
  if (rank == 0) {
    std::cout << "=== Running ndarray PNetCDF Auto I/O Tests ===" << std::endl;
    std::cout << "Running with " << nprocs << " MPI processes" << std::endl;
  }

  const std::string filename = "test_auto_pnetcdf.nc";
  ...
  // Test 1: Distributed Write
  {
    ...
    darray.write_pnetcdf_auto(filename, "test_var");
  ...
  // Test 2: Distributed Read
  {
    ...
    ftk::ndarray<float> darray;
    // Decompose 2D domain (same way)
    darray.decompose(MPI_COMM_WORLD, {nx, ny});
    
    if (rank == 0) std::cout << "    - Reading distributed array from " << filename << std::endl;
    darray.read_pnetcdf_auto(filename, "test_var");
  ...
  // Test 3: Replicated Read
  {
    ...
    rarray.reshapef(nx, ny); // Set desired dimensions
    
    if (rank == 0) std::cout << "    - Reading replicated array (all ranks get full data)" << std::endl;
    rarray.read_pnetcdf_auto(filename, "test_var");

    // Verify values (every rank checks full domain)
    bool correct = true;
  ...
  if (rank == 0) {
    ...
    std::cout << "\n=== All PNetCDF Auto I/O Tests Passed ===\n" << std::endl;
  }
...
#else
  std::cout << "PNetCDF Auto I/O tests require MPI and PNetCDF support" << std::endl;
  return 0;
#endif
}
